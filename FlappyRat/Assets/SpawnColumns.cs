...
public class SpawnColumns : MonoBehaviour
{
    ...
    [SerializeField]
    private float minGapCenterY = -1f;

    [SerializeField]
    private float maxGapCenterY = 2f;
    ...
    void SpawnColumn()
    {
        ...
        if (coinPrefab != null)
        {
            float coinY = Random.Range(gapBottom + 0.3f, gapTop - 0.3f);
            Instantiate(coinPrefab, new Vector3(19, coinY, 0), Quaternion.identity);
        }
    }

    public void StopSpawning()
    {
        CancelInvoke(nameof(SpawnColumn));
    }
}
